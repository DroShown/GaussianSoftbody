...
namespace GaussianSplatting.Editor
{
    ...
    public class GaussianSplatAssetCreator : EditorWindow
    {
        ...
        struct CalcChunkDataJob : IJobParallelFor
        {
            ...
            public void Execute(int chunkIdx)
            {
                ...
                for (int i = splatBegin; i < splatEnd; ++i)
                {
                    ...
                    s.scale = ((float2)s.scale - chunkMinscl) / (chunkMaxscl - chunkMinscl);
        ...
        static ulong EncodeFloat3ToNorm16(float2 v) // 48 bits: 16.16.16
        {
            return (ulong)(v.x * 65535.5f) | ((ulong)(v.y * 65535.5f) << 16);
        }
        static uint EncodeFloat3ToNorm11(float2 v) // 32 bits: 11.10.11
        {
            return (uint)(v.x * 2047.5f) | ((uint)(v.y * 1023.5f) << 11);
        }
        static ushort EncodeFloat3ToNorm655(float2 v) // 16 bits: 6.5.5
        {
            return (ushort)((uint)(v.x * 63.5f) | ((uint)(v.y * 31.5f) << 6));
        }
        ...
        static unsafe void EmitEncodedVector(float2 v, byte* outputPtr, GaussianSplatAsset.VectorFormat format)
        {
            switch (format)
            {
                case GaussianSplatAsset.VectorFormat.Float32:
